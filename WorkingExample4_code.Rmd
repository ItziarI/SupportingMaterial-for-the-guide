...
- **Step 1** Order  the PRSs according to the sum of  the Tjur's coefficients of discrimination. If Nagelkerke's R^2 i prefered, set statistic="PseudoR2" in function orderBin()
```{r}
#-----------------------------------------------
# Order the PRSs
...
```
The plots suggest that the interaction between the PRS.7 and the diagnostic is relevant. Thus, we set the full model candidate (FM): $Trait \sim PRS + Sex + Diagnostic + PRS*Diagnostic + Age + PC1 +PC2$.
...
With this p-value = 0.3389 we conclude that there is not evedence of overdispersion

- Check association between PRS.7 and Trait
```{r}
summary(FM)

```

The results do not indicate a clear association between PRS.7 and the Trait


\begin{itemize}
    \item $\widehat{log(p/1-p)} = 1.953 + 0.008\times PRS.7 + 0.446\times Sex + 0.044\times Age - 5.566\times PC1 + 3.785\times PC2$ if Diagnostic = 0.
    \item $\widehat{log(p/1-p)} = (1.953+16.714) + (0.008+0.162)\times PRS.7 + 0.446\times Sex + 0.044\times Age - 5.566\times PC1 + 3.785\times PC2$ if Diagnostic = 1.
\end{itemize}

That means that for those with Diagnostic=0, it seems that the PRS.7 is not related to the Trait with odds = exp(0.0082) = 1.008, but for those with Diagnosis =1 the model indicates that the coefficient of PRS.7 is 0.0082 + 0.162 = 0.1702, so the odds increase exp(0.1702) = 1.186 for an incremental of one unit in PRS.7.
