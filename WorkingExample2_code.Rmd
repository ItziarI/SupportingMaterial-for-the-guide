...
ggplot(data=out[sel,], aes(y=rank, x=effect, xmin=lower, xmax=upper)) +
  ...
  scale_y_continuous(name=NULL, breaks= n:1, labels=row.names(out), position="right") +
  labs(title='', x='Mean R^2', y = 'PRS') +
  geom_vline(xintercept=quantile(out$Sum, prob=seq(0, 1, length = 21))[20], color='red', linetype='solid') +
  geom_vline(xintercept=quantile(out$Sum, prob=seq(0, 1, length = 21))[19], color='red', linetype='dashed') +
    geom_vline(xintercept=quantile(out$Sum, prob=seq(0, 1, length = 21))[18], color='red', linetype='dashed') +
    geom_vline(xintercept=quantile(out$Sum, prob=seq(0, 1, length = 21))[17], color='red', linetype='dashed') +
  theme_minimal()
...
```{r}
#--------------------------------------------------------------------
sel_95 <- out$Sum > quantile(out$Sum, prob=seq(0, 1, length = 21))[20]
write.csv2(out[sel_95,-9], file=paste0(mainfilename, "_Percentile_95.csv"))
sel_90 <- out$Sum > quantile(out$Sum, prob=seq(0, 1, length = 21))[19]
write.csv2(out[sel_90,-9], file=paste0(mainfilename, "_Percentile_90.csv"))
sel_85 <- out$Sum > quantile(out$Sum, prob=seq(0, 1, length = 21))[18]
write.csv2(out[sel_85,-9], file=paste0(mainfilename, "_Percentile_85.csv"))
sel_80 <- out$Sum > quantile(out$Sum, prob=seq(0, 1, length = 21))[17]
...
```{r}
#Levene's test
car::leveneTest(.stdresid ~ Diagnostic, data=d)
```
...
The results show that PRS.5 is related with the Trait in the following way:
...
That means that for Diagnostic 1, the association is negative (coeff = -0.25979); for Diagnostic 0, there is not association (coeff = 0.00089). Given this transformation, the increase of one unit in PRS.5, makes the ration $\widehat{log(Trait)}|PRS.5 = 1/\widehat{log(Trait)|PRS.5=0} = exp(-0.25979)=0.771 < 1$, thus, gives a mean decrease of about 22.9\%.
