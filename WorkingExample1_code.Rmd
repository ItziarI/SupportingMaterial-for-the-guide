...
In this example we simulate 9 PRSs, and a continuous trait, with gender, clinical diagnosis (with 3 categories), age, and two Principal Components as covariates. 
...
According to the obtained results, first PRS.4 is selected to analyse its association with the Trait.
...
- **Step 3**. Check the significance of PRS.4
...
\begin{itemize}
    \item $\widehat{Trait} = 10.608 + 3.794\times PRS.4 + 0.117\times Sex + 0.0009\times Age + 3.064\times PC1 + 4.545\times PC2$ if Diagnostic = 1.
    \item $\widehat{Trait}$ = (10.608+1.582) + (3.794-3.922)\times PRS.4 + 0.117\times Sex + 0.009\times Age + 3.064\times PC1 + 4.545\timesPC2 if Diagnostic = 2.
    \item $\widehat{Trait}$ = (10.608+0.372) + (3.794-6.784)\times PRS.4 + 0.117\times\times Sex + 0.009\times Age + 3.064\times PC1 + 4.545\timesPC2 if Diagnostic = 3.
\end{itemize}

That means that for Diagnostic 1, the association is positive and strong (coeff = 3.794), for Diagnostic 2, there is not association (coeff = -0.132) and that for Diagnostic 3 the association is strong but negative (coeff = -2.990).
